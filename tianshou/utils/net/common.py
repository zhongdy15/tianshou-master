...
class ConvMLP(nn.Module):
    """ConvMLP based on Simple MLP backbone.
    ...
    * hidden_sizes[-1] * output_dim
    ...
    def __init__(
        ...
        model = []
        model += [nn.Conv2d(3, 32, 8, stride=4)]
        model += [nn.ReLU()]
        model += [nn.Conv2d(32, 64, 4, stride=2)]
        model += [nn.ReLU()]
        model += [nn.Conv2d(64, 64, 3, stride=1)]
        model += [nn.ReLU()]
        model += [nn.Flatten()]

        hidden_sizes = [64 *7*7] + list(hidden_sizes)
        for in_dim, out_dim, norm, activ in zip(
            hidden_sizes[:-1], hidden_sizes[1:], norm_layer_list, activation_list
        ):
            model += miniblock(in_dim, out_dim, norm, activ, linear_layer)
...
class ConvNet(nn.Module):
    ...
    def __init__(
        ...
        output_dim = action_dim if not self.use_dueling and not concat else 0
        self.model = ConvMLP(
            state_shape, output_dim, hidden_sizes, norm_layer, activation, device
        )
        self.output_dim = self.model.output_dim
        if self.use_dueling:  # dueling DQN
            q_kwargs, v_kwargs = dueling_param  # type: ignore
