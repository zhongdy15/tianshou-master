...
class RunningMan(gym.Env):
    ...
    def get_info(self):
        ...
        info['size'] = self.size
        # 目标隔几列出现一次
        info['dist_interval'] = self.dist_interval
        # agent初始位置集合
        info['initial_space'] = copy.deepcopy(self.initial_space)

        info['n_actions'] = self.action_space.n

        # 智能体总共能动多少次
        info['initial_chances'] = self.initial_chances
        info['chances'] = self.chances
        # 状态维度
        info['states_dim'] = self.states_dim
        # state:智能体当前的位置
        info['state'] = copy.deepcopy(self.state)
        # obs:输入神经网络的状态：【游戏进度,相对位置的x差值，相对位置的y差值，剩余能动字数比例】
        info['obs'] = copy.deepcopy(self.obs)
        # 障碍物点的集合
        info['target'] = copy.deepcopy(self.target)
        info['viewer'] = None
        # 游戏最大长度
        info['max_step'] = self.max_step
        # 游戏步数
        ...
        # 环境可以放置困难的list:
        info['target_pos_num'] = []#copy.deepcopy(self.target_pos_num)
        info['challenge_index'] = copy.deepcopy(self.challenge_index)

        info['action_penalty'] = copy.deepcopy(self.action_penalty)
        return info

    def copy_as_infolist(self, infolist):
        # todo:
        # 按照infolist里给定的信息重建整个环境

        #print(infolist)
        info = infolist
        self.action_space = info['action_space']
        self.observation_space = info['observation_space']
        # 地图的长宽为多少个格点
        ...
        self.size = info['size']
        # 目标隔几列出现一次
        self.dist_interval = info['dist_interval']

        self.initial_space = copy.deepcopy(info['initial_space'])

        self.n_actions = info['n_actions']

        # 智能体总共能动多少次
        self.initial_chances = info['initial_chances']
        self.chances = info['chances']
        # 状态维度
        self.states_dim = info['states_dim']
        # state:智能体当前的位置
        self.state = copy.deepcopy(info['state'])
        # obs:输入神经网络的状态：【游戏进度,相对位置的x差值，相对位置的y差值，剩余能动字数比例】
        self.obs = copy.deepcopy(info['obs'])
        # 障碍物点的集合
        self.target = copy.deepcopy(info['target'])
        self.viewer = None
        # 游戏最大长度
        ...
        # 环境可以放置困难的list:
        self.target_pos_num = []#copy.deepcopy(info['target_pos_num'])
        self.challenge_index = copy.deepcopy(info['challenge_index'])

        self.action_penalty = copy.deepcopy(info['action_penalty'])
